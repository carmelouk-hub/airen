...
BEGIN
  ...
  IF v_po_supplier<>v_item_supplier OR NEW.ingredient_id<>v_item_ingredient OR NEW.uom_id<>v_item_uom THEN RAISE EXCEPTION 'PURCHASE_ORDER_LINE_MATERIAL_MISMATCH'; END IF;
  ...
  RETURN NEW;
END $$;
CREATE TRIGGER trg_risto_purchase_order_line_validate BEFORE INSERT ON ristoairen.purchase_order_lines FOR EACH ROW EXECUTE FUNCTION ristoairen.validate_purchase_order_line();

CREATE OR REPLACE FUNCTION ristoairen.validate_po_backed_goods_receipt_insert()
RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
  ...
  SELECT status,supplier_id,environment_class INTO v_status,v_supplier,v_environment
    FROM ristoairen.purchase_orders
   WHERE tenant_id=NEW.tenant_id AND location_id=NEW.location_id AND id=NEW.purchase_order_id;
  IF v_status NOT IN ('APPROVED','SENT','PARTIALLY_RECEIVED') THEN RAISE EXCEPTION 'PURCHASE_ORDER_NOT_RECEIVABLE'; END IF;
  IF NEW.supplier_id IS DISTINCT FROM v_supplier THEN RAISE EXCEPTION 'GOODS_RECEIPT_PURCHASE_ORDER_SUPPLIER_MISMATCH'; END IF;
  IF NEW.environment_class IS DISTINCT FROM v_environment THEN RAISE EXCEPTION 'GOODS_RECEIPT_PURCHASE_ORDER_ENVIRONMENT_MISMATCH'; END IF;
  RETURN NEW;
END $$;
CREATE TRIGGER trg_risto_po_backed_goods_receipt_insert BEFORE INSERT ON ristoairen.goods_receipts FOR EACH ROW EXECUTE FUNCTION ristoairen.validate_po_backed_goods_receipt_insert();
...
BEGIN
  SELECT purchase_order_id,supplier_id,status,environment_class INTO v_po,v_receipt_supplier,v_receipt_status,v_receipt_environment FROM ristoairen.goods_receipts WHERE tenant_id=NEW.tenant_id AND location_id=NEW.location_id AND id=NEW.goods_receipt_id;
  ...
  IF v_po IS NULL THEN IF NEW.purchase_order_line_id IS NOT NULL OR NEW.unit_cost_actual IS NOT NULL THEN RAISE EXCEPTION 'FREE_RECEIPT_CANNOT_ASSERT_PURCHASE_ORDER_LINE'; END IF; RETURN NEW; END IF;
  IF NEW.purchase_order_line_id IS NULL OR NEW.unit_cost_actual IS NULL THEN RAISE EXCEPTION 'PO_RECEIPT_LINE_REQUIRES_ORDER_LINE_AND_PRICE'; END IF;
  SELECT status,supplier_id INTO v_po_status,v_po_supplier FROM ristoairen.purchase_orders WHERE tenant_id=NEW.tenant_id AND location_id=NEW.location_id AND id=v_po;
  IF v_po_status NOT IN ('APPROVED','SENT','PARTIALLY_RECEIVED') OR v_po_supplier<>v_receipt_supplier THEN RAISE EXCEPTION 'PURCHASE_ORDER_NOT_RECEIVABLE'; END IF;
  SELECT purchase_order_id,ingredient_id,uom_id,quantity_ordered INTO v_line_po,v_line_ingredient,v_line_uom,v_quantity_ordered FROM ristoairen.purchase_order_lines WHERE tenant_id=NEW.tenant_id AND location_id=NEW.location_id AND id=NEW.purchase_order_line_id;
  IF v_line_po IS DISTINCT FROM v_po OR NEW.ingredient_id IS DISTINCT FROM v_line_ingredient OR NEW.uom_id IS DISTINCT FROM v_line_uom THEN RAISE EXCEPTION 'GOODS_RECEIPT_LINE_PURCHASE_ORDER_MISMATCH'; END IF;
  SELECT COALESCE(sum(grl.quantity_received),0) INTO v_confirmed FROM ristoairen.goods_receipt_lines grl JOIN ristoairen.goods_receipts gr ON gr.tenant_id=grl.tenant_id AND gr.location_id=grl.location_id AND gr.id=grl.goods_receipt_id WHERE grl.tenant_id=NEW.tenant_id AND grl.location_id=NEW.location_id AND grl.purchase_order_line_id=NEW.purchase_order_line_id AND gr.status='CONFIRMED';
  IF v_confirmed+NEW.quantity_received>v_quantity_ordered THEN RAISE EXCEPTION 'PURCHASE_ORDER_OVER_RECEIPT'; END IF;
  RETURN NEW;
END $$;
CREATE TRIGGER trg_risto_procurement_receipt_line_validate BEFORE INSERT ON ristoairen.goods_receipt_lines FOR EACH ROW EXECUTE FUNCTION ristoairen.validate_procurement_receipt_line();
...
BEGIN
  ...
  IF NEW.tenant_id<>OLD.tenant_id OR NEW.location_id<>OLD.location_id OR NEW.supplier_id<>OLD.supplier_id OR NEW.purchase_order_id IS DISTINCT FROM OLD.purchase_order_id OR NEW.receipt_number<>OLD.receipt_number OR NEW.received_at<>OLD.received_at OR NEW.received_by_identity_id<>OLD.received_by_identity_id OR NEW.source_request_key<>OLD.source_request_key OR NEW.environment_class<>OLD.environment_class THEN RAISE EXCEPTION 'GOODS_RECEIPT_IDENTITY_IMMUTABLE'; END IF;
  IF NEW.status<>'CONFIRMED' OR NEW.confirmed_at IS NULL OR NEW.confirmed_by_identity_id IS NULL OR NEW.row_version<>OLD.row_version+1 THEN RAISE EXCEPTION 'GOODS_RECEIPT_INVALID_CONFIRMATION'; END IF;
  IF NEW.purchase_order_id IS NOT NULL THEN
    SELECT status,supplier_id INTO v_po_status,v_po_supplier FROM ristoairen.purchase_orders WHERE tenant_id=NEW.tenant_id AND location_id=NEW.location_id AND id=NEW.purchase_order_id;
    IF v_po_status NOT IN ('APPROVED','SENT','PARTIALLY_RECEIVED') OR v_po_supplier<>NEW.supplier_id THEN RAISE EXCEPTION 'PURCHASE_ORDER_NOT_RECEIVABLE'; END IF;
    IF NOT EXISTS (SELECT 1 FROM ristoairen.goods_receipt_lines l WHERE l.tenant_id=NEW.tenant_id AND l.location_id=NEW.location_id AND l.goods_receipt_id=NEW.id) THEN RAISE EXCEPTION 'GOODS_RECEIPT_REQUIRES_LINES'; END IF;
    SELECT EXISTS (
      SELECT 1 FROM ristoairen.goods_receipt_lines l
      LEFT JOIN ristoairen.purchase_order_lines pol ON pol.tenant_id=l.tenant_id AND pol.location_id=l.location_id AND pol.id=l.purchase_order_line_id
      WHERE l.tenant_id=NEW.tenant_id AND l.location_id=NEW.location_id AND l.goods_receipt_id=NEW.id
        AND (l.purchase_order_line_id IS NULL OR l.unit_cost_actual IS NULL OR pol.purchase_order_id IS DISTINCT FROM NEW.purchase_order_id OR pol.ingredient_id IS DISTINCT FROM l.ingredient_id OR pol.uom_id IS DISTINCT FROM l.uom_id
          OR COALESCE((SELECT sum(prev.quantity_received) FROM ristoairen.goods_receipt_lines prev JOIN ristoairen.goods_receipts pgr ON pgr.tenant_id=prev.tenant_id AND pgr.location_id=prev.location_id AND pgr.id=prev.goods_receipt_id WHERE prev.tenant_id=l.tenant_id AND prev.location_id=l.location_id AND prev.purchase_order_line_id=l.purchase_order_line_id AND pgr.status='CONFIRMED'),0)+l.quantity_received>pol.quantity_ordered)
    ) INTO v_invalid;
...
BEGIN
  SELECT pol.supplier_item_id,grl.unit_cost_actual,po.currency,gr.received_at,gr.environment_class INTO v_supplier_item,v_cost,v_currency,v_effective,v_environment
    FROM ristoairen.goods_receipt_lines grl
    JOIN ristoairen.goods_receipts gr ON gr.tenant_id=grl.tenant_id AND gr.location_id=grl.location_id AND gr.id=grl.goods_receipt_id
    JOIN ristoairen.purchase_order_lines pol ON pol.tenant_id=grl.tenant_id AND pol.location_id=grl.location_id AND pol.id=grl.purchase_order_line_id
    JOIN ristoairen.purchase_orders po ON po.tenant_id=pol.tenant_id AND po.location_id=pol.location_id AND po.id=pol.purchase_order_id
  ...
  IF NEW.supplier_item_id<>v_supplier_item OR NEW.unit_cost<>v_cost OR NEW.currency<>v_currency OR NEW.effective_at<>v_effective OR NEW.environment_class<>v_environment THEN RAISE EXCEPTION 'SUPPLIER_PRICE_SOURCE_MISMATCH'; END IF;
  ...
  RETURN NEW;
END $$;
CREATE TRIGGER trg_risto_supplier_price_validate BEFORE INSERT ON ristoairen.supplier_prices FOR EACH ROW EXECUTE FUNCTION ristoairen.validate_supplier_price();
...
BEGIN
  IF NEW.tenant_id<>OLD.tenant_id OR NEW.location_id<>OLD.location_id OR NEW.supplier_id<>OLD.supplier_id OR NEW.order_number<>OLD.order_number OR NEW.order_date<>OLD.order_date OR NEW.expected_delivery_at IS DISTINCT FROM OLD.expected_delivery_at OR NEW.currency<>OLD.currency OR NEW.subtotal<>OLD.subtotal OR NEW.tax_total IS DISTINCT FROM OLD.tax_total OR NEW.total<>OLD.total OR NEW.created_by_identity_id<>OLD.created_by_identity_id OR NEW.approved_by_identity_id IS DISTINCT FROM OLD.approved_by_identity_id OR NEW.approved_at IS DISTINCT FROM OLD.approved_at OR NEW.sent_at IS DISTINCT FROM OLD.sent_at OR NEW.closed_at IS DISTINCT FROM OLD.closed_at OR NEW.source_type<>OLD.source_type OR NEW.source_suggestion_id IS DISTINCT FROM OLD.source_suggestion_id OR NEW.environment_class<>OLD.environment_class OR NEW.created_at<>OLD.created_at THEN RAISE EXCEPTION 'PURCHASE_ORDER_RECEIVING_IDENTITY_IMMUTABLE'; END IF;
  IF OLD.status NOT IN ('APPROVED','SENT','PARTIALLY_RECEIVED') OR NEW.status NOT IN ('PARTIALLY_RECEIVED','RECEIVED') OR NEW.row_version<>OLD.row_version+1 THEN RAISE EXCEPTION 'PURCHASE_ORDER_INVALID_RECEIVING_TRANSITION'; END IF;
  SELECT count(*)::int,bool_or(COALESCE(r.received,0)>0),bool_and(COALESCE(r.received,0)=pol.quantity_ordered)
  ...
  RETURN NEW;
END $$;
CREATE TRIGGER trg_risto_purchase_order_receiving_guard BEFORE UPDATE ON ristoairen.purchase_orders FOR EACH ROW EXECUTE FUNCTION ristoairen.guard_purchase_order_receiving_update();
...
CREATE POLICY risto_supplier_item_tenant_policy ON ristoairen.supplier_items FOR SELECT TO airen_app USING (tenant_id=NULLIF(current_setting('airen.tenant_id',true),'')::uuid);
CREATE POLICY risto_purchase_order_scope_policy ON ristoairen.purchase_orders FOR ALL TO airen_app USING (tenant_id=NULLIF(current_setting('airen.tenant_id',true),'')::uuid AND location_id=NULLIF(current_setting('airen.location_id',true),'')::uuid) WITH CHECK (tenant_id=NULLIF(current_setting('airen.tenant_id',true),'')::uuid AND location_id=NULLIF(current_setting('airen.location_id',true),'')::uuid);
CREATE POLICY risto_purchase_order_line_scope_policy ON ristoairen.purchase_order_lines FOR SELECT TO airen_app USING (tenant_id=NULLIF(current_setting('airen.tenant_id',true),'')::uuid AND location_id=NULLIF(current_setting('airen.location_id',true),'')::uuid);
CREATE POLICY risto_supplier_price_tenant_policy ON ristoairen.supplier_prices FOR ALL TO airen_app USING (tenant_id=NULLIF(current_setting('airen.tenant_id',true),'')::uuid) WITH CHECK (tenant_id=NULLIF(current_setting('airen.tenant_id',true),'')::uuid);
