-- R3-D correction after CI #355 / run 32590462013.
-- Cause 1: airen_control_plane_owner had SELECT on identity.identities but lacked USAGE on schema identity.
-- Cause 2: revoke on a missing assignment relied on SQL NULL semantics instead of an explicit fail-closed state check.
-- This migration does not grant table authority to airen_control_plane or airen_app.
BEGIN;
