-- R3-D third correction after CI #373 / run 32592178660.
-- PostgreSQL exposed PL/pgSQL output-variable/table-column ambiguity for role_key.
-- Correction is syntactic qualification only: lifecycle, permissions and grants are unchanged.
BEGIN;
...
BEGIN
  ...
  IF p_action='assign' THEN
    ...
    ELSIF v_current_status='revoked' THEN
      UPDATE authz.platform_role_assignments AS pra SET status='active',updated_at=now()
      WHERE pra.identity_id=p_target_identity_id AND pra.role_key=p_role_key
      RETURNING pra.status,pra.created_at,pra.updated_at INTO v_current_status,v_created_at,v_updated_at;
  ...
  ELSIF p_action='suspend' THEN
    ...
    END IF;
    UPDATE authz.platform_role_assignments AS pra SET status='suspended',updated_at=now()
    WHERE pra.identity_id=p_target_identity_id AND pra.role_key=p_role_key
    RETURNING pra.status,pra.created_at,pra.updated_at INTO v_current_status,v_created_at,v_updated_at;
  ...
  ELSE
    ...
    IF v_is_protected AND v_current_status='active' THEN
      ...
      IF v_active_count-1 < v_protected_min THEN RAISE EXCEPTION 'AIRENOS_PROTECTED_ROLE_MINIMUM_ACTIVE' USING ERRCODE='P0001'; END IF;
    END IF;
    UPDATE authz.platform_role_assignments AS pra SET status='revoked',updated_at=now()
    WHERE pra.identity_id=p_target_identity_id AND pra.role_key=p_role_key
    RETURNING pra.status,pra.created_at,pra.updated_at INTO v_current_status,v_created_at,v_updated_at;
    v_reason := p_reason_code;
  END IF;
