...
function required(environment: EnvironmentInput, key: string): string {
  ...
  if (!value) throw new AppError("RUNTIME_CONFIGURATION_INVALID", `Missing required Foundation Attachment staging environment field: ${key}`, { field: key });
  return value;
}
...
export function createRa01AirenOSAuthentication(
  ...
  if (environment.AUTH_ADAPTER?.trim() !== "airenos-session-ed25519") {
    throw new AppError("RUNTIME_CONFIGURATION_INVALID", "Foundation Attachment staging requires canonical AIRenOS Session Authority authentication", { field: "AUTH_ADAPTER" });
  }
...
export async function startFoundationAttachmentHttpServer(environment: EnvironmentInput = process.env) {
  ...
  const pool = runtime.withDatabaseConnectionString((connectionString) => new Pool({
    connectionString,
    max: 5,
    application_name: "airenos-foundation-attachment-staging",
  ...
  const attachmentProjectionPublisher = createPilotAttachmentProjectionPublisher({ environment, entitlements: entitlementStore });
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      if (request.method === "GET" && request.url === "/health/live") {
        json(response, 200, { status: "LIVE", service: "airenos-foundation-attachment-staging", releaseRevision: deployment.releaseRevision });
        return;
      }
