...
for (const required of ["CREATE TABLE platform.tenant_provisioning_idempotency", "platform.tenants.provision", "security.platform_provision_tenant", "SECURITY DEFINER", "OWNER TO airen_control_plane_owner", "REVOKE ALL ON FUNCTION security.platform_provision_tenant", "GRANT EXECUTE ON FUNCTION security.platform_provision_tenant", "TO airen_control_plane"]) if (!r3a.includes(required)) throw new Error(`Missing R3-A SQL capability fragment: ${required}`);
if (/GRANT\s+(?:SELECT|INSERT|UPDATE|DELETE)[\s\S]{0,160}ON\s+(?:platform\.tenants|platform\.locations|authz\.tenant_memberships|audit\.audit_events|events\.outbox_events)[\s\S]{0,160}TO\s+airen_control_plane\b/i.test(r3a)) throw new Error("airen_control_plane must not receive direct table mutation grants");
