...
export type GoodsReceiptRecord = Readonly<{id:string;tenantId:string;locationId:string;supplierId:string;receiptNumber:string;receivedAt:string;receivedByIdentityId:string;status:GoodsReceiptStatus;sourceRequestKey:string;rowVersion:number;environmentClass:InventoryEnvironmentClass;confirmedAt?:string;confirmedByIdentityId?:string}>;
export type GoodsReceiptLineRecord = Readonly<{id:string;tenantId:string;locationId:string;goodsReceiptId:string;ingredientId:string;uomId:string;quantityReceived:string;rejectedQuantity:string;sourceLineKey:string;rowVersion:number;environmentClass:InventoryEnvironmentClass}>;
...
export interface InventoryReceiveTransaction extends TransactionContext {
  ...
  insertReceipt(input:Readonly<{tenantId:string;locationId:string;supplierId:string;receiptNumber:string;receivedAt:string;receivedByIdentityId:string;sourceRequestKey:string;environmentClass:InventoryEnvironmentClass}>):Promise<GoodsReceiptRecord>;
  ...
  insertLine(input:Readonly<{tenantId:string;locationId:string;goodsReceiptId:string;ingredientId:string;uomId:string;quantityReceived:string;sourceLineKey:string;environmentClass:InventoryEnvironmentClass}>):Promise<GoodsReceiptLineRecord>;
...
export type InventoryReceiveDependencies=Readonly<{unitOfWork:UnitOfWork<InventoryReceiveTransaction>;now?:()=>string;environmentClass?:InventoryEnvironmentClass}>;
...
function requireEntitlement(context:SecurityContext):void{if(!context.entitlements.includes(RISTOAIREN_ENTITLEMENT))throw new AppError("ENTITLEMENT_REQUIRED",`Missing entitlement: ${RISTOAIREN_ENTITLEMENT}`);}
...
function authority(context:SecurityContext,permission:string):void{requireEntitlement(context);requireLocationAuthority(context);requirePermission(context,permission,{tenantId:context.tenantId,locationId:context.locationId});}
...
export async function createGoodsReceipt(context:SecurityContext,rawInput:Readonly<{supplierId:string;receiptNumber:string;receivedAt?:string;idempotencyKey:string}>,dependencies:InventoryReceiveDependencies):Promise<Readonly<{receipt:GoodsReceiptRecord;replayed:boolean}>>{
  ...
  const explicitReceivedAt=rawInput.receivedAt?timestamp(rawInput.receivedAt,"receivedAt"):undefined;
  const receivedAt=explicitReceivedAt??serverNow(dependencies.now);
  return dependencies.unitOfWork.transaction(async tx=>{
    const existing=await tx.findReceiptByRequestKey(sourceRequestKey);
    if(existing){
      if(existing.supplierId!==supplierId||existing.receiptNumber!==receiptNumber||(explicitReceivedAt!==undefined&&existing.receivedAt!==explicitReceivedAt))conflict("GoodsReceipt idempotency key was already used with different input");
      return Object.freeze({receipt:existing,replayed:true});
    }
...
export async function recordGoodsReceiptLine(context:SecurityContext,rawInput:Readonly<{goodsReceiptId:string;ingredientId:string;uomId:string;quantityReceived:string;idempotencyKey:string}>,dependencies:InventoryReceiveDependencies):Promise<Readonly<{line:GoodsReceiptLineRecord;replayed:boolean}>>{
  ...
  return dependencies.unitOfWork.transaction(async tx=>{
    const receipt=await tx.getReceiptForLine(goodsReceiptId);if(!receipt)notFound("GoodsReceipt not found");if(receipt.status!=="DRAFT")conflict("Only DRAFT GoodsReceipt can accept lines");
    const existing=await tx.findLineByRequestKey(goodsReceiptId,sourceLineKey);
    if(existing){if(existing.ingredientId!==ingredientId||existing.uomId!==uomId||existing.quantityReceived!==quantityReceived)conflict("GoodsReceiptLine idempotency key was already used with different input");return Object.freeze({line:existing,replayed:true});}
...
export async function finalizeGoodsReceipt(context:SecurityContext,rawInput:Readonly<{goodsReceiptId:string;expectedRowVersion:number}>,dependencies:InventoryReceiveDependencies):Promise<Readonly<{receipt:GoodsReceiptRecord;movements:readonly StockMovementRecord[];stockItems:readonly StockItemRecord[];replayed:boolean}>>{
  ...
  return dependencies.unitOfWork.transaction(async tx=>{
    const receipt=await tx.getReceiptForFinalize(goodsReceiptId);if(!receipt)notFound("GoodsReceipt not found");
    ...
    const stockItems=await tx.listReceiptStockItems(goodsReceiptId);await tx.audit(audit(context,GOODS_RECEIPT_FINALIZED_ACTION,"GoodsReceipt",confirmed.id,Object.freeze({movementCount:movements.length,movementIds:movements.map(m=>m.id),source:"GoodsReceipt"})));
