...
## Render F2.6 staging adapter

`render.identity.keycloak.f26.yaml` adapts this provider-neutral image to the approved Render Frankfurt staging boundary. The adapter uses one Standard 2 GB instance, optimized production mode, Render edge TLS, a main HTTP listener on port 10000 and `/health/ready` on that listener. Automatic deployment is disabled.

The staging adapter consumes only externally supplied database and bootstrap-administrator secrets. Its database is a separate logical PostgreSQL database with a dedicated Keycloak runtime principal; it does not share the AIRenOS Identity logical database, schema, owner or runtime principal. This single-instance staging adapter is not the production topology and does not waive the production replica, database resilience, administrative-hostname or network-restriction requirements above.
